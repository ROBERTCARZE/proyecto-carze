/* ==========================================================================
   CERTIFICADOS.JS — Lógica del módulo de Certificados
   CARZE Contratistas Generales S.A.C.
   --------------------------------------------------------------------------
   Extraído de certificados.html tal cual estaba (mismo comportamiento, cero
   cambios de lógica) — noveno módulo separado. Incluye el campo de
   Presupuesto de Levantamiento de Observaciones agregado hoy.
   ========================================================================== */
...
const DOC_FIELDS = [
    ...
    {key:'cert', emoji:'🏅', label:'Certificado',          hint:'Nombre del certificado'},
    {key:'inf',  emoji:'📄', label:'Informe',              hint:'Nombre del informe'},
    {key:'acta', emoji:'✅', label:'Acta',                 hint:'Nombre del acta'},
    {key:'preslev', emoji:'🛠️', label:'Presupuesto Levant. Observ.', hint:'Solo si el servicio fue Observado'},
...
// Ícono compacto junto al pill "Observado" — solo aparece cuando el estado es Observado.
// No agrega columna: vive dentro de la misma celda de Estado.
function preslevBadge(row){
    var est=(row.estado||'').toLowerCase();
    if(est!=='observado') return '';
    var tiene = row.preslevNombre && row.preslevNombre!=='-' && row.preslevLink && row.preslevLink!=='-';
    if(tiene){
        return '<a href="'+String(row.preslevLink).replace(/"/g,'&quot;')+'" target="_blank" rel="noopener" '+
               'title="Presupuesto levant. observ.: '+String(row.preslevNombre).replace(/"/g,'&quot;')+'" '+
               'style="margin-left:6px;display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;'+
               'border-radius:6px;background:#fff7ed;border:1px solid #fdba74;text-decoration:none;font-size:.68rem;vertical-align:middle" '+
               'onclick="event.stopPropagation()">🛠️</a>';
    }
    return '<span title="Sin presupuesto de levantamiento cargado" '+
           'style="margin-left:6px;display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;'+
           'border-radius:6px;background:#f8fafc;border:1px dashed #cbd5e1;opacity:.55;font-size:.68rem;vertical-align:middle">🛠️</span>';
}
...
function renderTabla(data){
    ...
    data.slice(ini,fin).forEach(function(row){
        ...
        tr.innerHTML=
            ...
            '<td>'+docLink(row.certNombre,row.certLink)+'</td>'+
            ...
            '<td>'+pillEstado(row.estado)+preslevBadge(row)+'</td>'+
...
function buildDocFields(){
    ...
    DOC_FIELDS.forEach(function(f){
        ...
        d.id='docsec-'+f.key;
        if(f.key==='preslev'){ d.style.display='none'; }
        d.innerHTML=
...
function handleFile(event, key){
    ...
    var path='certificados/'+num+'_'+key+'_'+Date.now()+'.'+ext;
...
function toggleDocPreslev(){
    var est=(document.getElementById('fEstado').value||'').toLowerCase();
    var sec=document.getElementById('docsec-preslev');
    if(!sec) return;
    var tieneCargado = !!(uploadState.preslev && uploadState.preslev.nombre);
    sec.style.display=(est==='observado'||tieneCargado)?'block':'none';
}
window.toggleDocPreslev = toggleDocPreslev;
...
function calcular(){
    ...
    var det=0;
    if(cond==='Servicio') det=total>=700?Math.round(total*0.12):0;
...
function abrirModal(){
    ...
    document.getElementById('fFecha').value=hoy();
    document.getElementById('modalTitle').innerHTML='Nuevo <span>Certificado</span>';
...
function editarSeleccionado(){
    ...
    document.getElementById('modalTitle').innerHTML='Editar <span>Certificado</span>';
    document.getElementById('btnGuardarTxt').textContent='ACTUALIZAR';
    document.getElementById('overlay').classList.add('open');
    cargarDocsExistentes(row); calcular(); toggleDocPreslev();
}
...
async function guardar(){
    ...
    var docData={
        ...
        certNombre:gDoc('cert'), certLink: gLink('cert'),
        infNombre: gDoc('inf'),  infLink:  gLink('inf'),
        actaNombre:gDoc('acta'), actaLink: gLink('acta'),
        preslevNombre:gDoc('preslev'), preslevLink: gLink('preslev'),
...
function pedirEliminar(docId,cliente){
    pendingDel=docId;
    document.getElementById('confirmMsg').textContent='Se eliminará el certificado de "'+cliente+'". Esta acción no se puede deshacer.';
...
async function ejecutarEliminar(){
    ...
    try{await deleteDoc(doc(db,COL,pendingDel));toast('Certificado eliminado','warn');}
...
function generarPDF(){
    ...
    document.getElementById('pdfTotalRegistros').textContent='Total: '+datosFiltrados.length+' certificados';
...
async function cargarZonas(){
    try {
        ...
        if(snap.empty){
            ...
            var raw = snap.docs.map(function(d){return {_id:d.id, nombre:d.data().nombre};});
            zonas = await dedupZonas(raw);
            zonas.sort();
        }
    } catch(e){
        zonas = [...ZONAS_BASE];
...
// Detecta zonas repetidas (mismo nombre) y borra las copias de más en
// Firestore, dejando solo una de cada una.
async function dedupZonas(raw){
    var vistos = {};
    var aBorrar = [];
    var unicos = [];
    raw.forEach(function(z){
        var key = String(z.nombre||'').trim().toLowerCase();
        if(!key) return;
        if(vistos[key]){
            aBorrar.push(z._id);
        } else {
            vistos[key] = true;
            unicos.push(z.nombre);
        }
    });
    for(var i=0;i<aBorrar.length;i++){
        try{ await deleteDoc(doc(db, COL_ZONAS, aBorrar[i])); } catch(e){}
    }
    if(aBorrar.length) toast('Se limpiaron '+aBorrar.length+' zona(s) duplicada(s)','warn');
    return unicos;
}
...
// cerrarSesion() ahora la expone auth-guard.js globalmente — no hace falta redefinirla aquí.
