...
async function ejecutarEliminar(){
    ...
    var idBorrar=pendingDel;
    cerrarConfirm();
    try{await deleteDoc(doc(db,COL,idBorrar));toast('Factura eliminada','warn');}
...
async function cargarClientes(){
    try {
        ...
        if(snap.empty){
            ...
            var raw = snap.docs.map(function(d){var r=d.data();r._id=d.id;return r;});
            clientes = await dedupClientes(raw);
        }
    } catch(e){
        clientes = [...CLIENTES_BASE];
    }
...
// Detecta clientes repetidos (mismo RUC, o mismo nombre si no hay RUC) y
// borra las copias de más en Firestore, dejando solo la primera de cada uno.
async function dedupClientes(raw){
    var vistos = {};
    var aBorrar = [];
    var unicos = [];
    raw.forEach(function(cli){
        var key = String(cli.ruc||cli.nombre||'').trim().toLowerCase();
        if(!key) { unicos.push(cli); return; }
        if(vistos[key]){
            aBorrar.push(cli._id);
        } else {
            vistos[key] = true;
            unicos.push(cli);
        }
    ...
    for(var i=0;i<aBorrar.length;i++){
        try{ await deleteDoc(doc(db, COL_CLI, aBorrar[i])); } catch(e){}
    }
    if(aBorrar.length) toast('Se limpiaron '+aBorrar.length+' cliente(s) duplicado(s)','warn');
    return unicos;
}
