...
// Copia intacta de las subcategorías de fábrica — nunca se toca. Sirve
// como base fija para combinar con las subcategorías personalizadas sin
// perder las originales (ver iniciarConfigListener).
const SUBCATS_BASE=JSON.parse(JSON.stringify(SUBCATS));
...
function iniciarConfigListener(){
    onSnapshot(configRef,function(snap){
        ...
        // Combina SIEMPRE desde la base de fábrica (SUBCATS_BASE) + las
        // personalizadas guardadas en Firestore, sin duplicados. Así nunca
        // se pierden las subcategorías originales, sin importar cuántas
        // veces se dispare este listener.
        var todasLasClaves={};
        Object.keys(SUBCATS_BASE).forEach(function(k){todasLasClaves[k]=true;});
        Object.keys(customCategorias).forEach(function(k){todasLasClaves[k]=true;});
        Object.keys(customSubcats).forEach(function(k){todasLasClaves[k]=true;});
        Object.keys(todasLasClaves).forEach(function(val){
            var base=SUBCATS_BASE[val]||[];
            var extra=customSubcats[val]||[];
            var combinado=base.slice();
            extra.forEach(function(s){ if(combinado.indexOf(s)<0) combinado.push(s); });
            SUBCATS[val]=combinado;
        });
...
function rebuildCategoriaOptions(){
    ...
    // También refresca el <select> de Subcategoría si ya hay una
    // categoría elegida (por ejemplo, el modal ya está abierto y acabas
    // de agregar una subcategoría nueva vía Firestore en tiempo real) —
    // si no se hace esto, la lista visible se queda "congelada" con las
    // opciones que tenía al momento de abrir el modal.
    var catSel=document.getElementById('mCategoria');
    if(catSel && catSel.value){
        var subSel=document.getElementById('mSubcategoria');
        var valSubActual=subSel.value;
        actualizarSubcat();
        if(valSubActual && subSel.querySelector('option[value="'+CSS.escape(valSubActual)+'"]')){
            subSel.value=valSubActual;
        }
    }
}
...
async function guardarNuevaCategoria(){
    ...
    try{
        // OJO: nunca usar setDoc(configRef,{customCategorias:{}},{merge:true})
        // aquí — eso BORRA por completo el campo customCategorias cada vez
        // (Firestore no combina objetos anidados con merge:true, los
        // reemplaza). En su lugar, se intenta el update directo y solo si
        // el documento no existe todavía (primera vez que se usa el
        // sistema) se crea con setDoc, sin tocar nada más.
        try{
            await updateDoc(configRef,{
                ['customCategorias.'+value]:{label:nombre,tipo:ncTipoSel,color:color.color,emoji:color.emoji}
            });
        }catch(errUpd){
            if(errUpd.code==='not-found'){
                await setDoc(configRef,{customCategorias:{}},{merge:false});
                await updateDoc(configRef,{
                    ['customCategorias.'+value]:{label:nombre,tipo:ncTipoSel,color:color.color,emoji:color.emoji}
                });
            } else { throw errUpd; }
...
async function guardarNuevaSubcategoria(){
    ...
    btn.disabled=true; var prevTxt=txt.textContent; txt.innerHTML='<span class="spinner"></span>';
    try{
        // Mismo cuidado que en guardarNuevaCategoria: nunca hacer
        // setDoc(configRef,{customSubcats:{}},{merge:true}) aquí, porque
        // eso reemplaza TODO el campo customSubcats y borra las
        // subcategorías que ya se habían agregado antes. Se intenta el
        // update directo, y solo si el documento no existe (primerísima
        // vez) se crea de cero.
        try{
            await updateDoc(configRef,{
            ...
            if(errUpd.code==='not-found'){
                await setDoc(configRef,{customSubcats:{}},{merge:false});
                await updateDoc(configRef,{
                    ['customSubcats.'+nsCategoriaActual]:arrayUnion(nombre)
                });
            } else { throw errUpd; }
