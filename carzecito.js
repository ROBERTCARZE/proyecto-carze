...
(function () {
  ...
  async function procesarMensaje(textoOriginal) {
    ...
    var pidioSaldo = textoNorm.indexOf('saldo') !== -1 ||
                      (textoNorm.indexOf('cuanto') !== -1 && (textoNorm.indexOf('caja') !== -1 || textoNorm.indexOf('dinero') !== -1));
    if (pidioSaldo) {
      ...
      if (saldo === null) {
        ...
        var aclaracion = paginaActual().indexOf('caja_diaria') === -1
          ? ' <span style="color:#94a3b8;font-size:.7rem">(dato de Caja Diaria — otras pantallas como Flujo de Caja pueden combinar más fuentes y mostrar una cifra distinta)</span>'
          : '';
        loader.innerHTML = `💵 El saldo actual en <b>Caja Diaria</b> es de <b>S/ ${saldo.toLocaleString('es-PE', { minimumFractionDigits: 2 })}</b>.${aclaracion}`;
      }
      return;
    }

    // C.1 Total histórico pagado en SCTR (dato real, sumado en JS — no Gemini)
    var pidioTotalSCTR = textoNorm.indexOf('sctr') !== -1 &&
      (textoNorm.indexOf('pagado') !== -1 || textoNorm.indexOf('total') !== -1 ||
       textoNorm.indexOf('suma') !== -1 || textoNorm.indexOf('cuanto hemos') !== -1 ||
       textoNorm.indexOf('cuanto se ha') !== -1 || textoNorm.indexOf('gastado') !== -1);

    if (pidioTotalSCTR) {
      var loaderSCTR = addMsg('<span class="cz-typing"></span> Sumando pagos de SCTR desde el inicio...', 'bot');
      if (!DB.disponible()) {
        loaderSCTR.innerHTML = '⚠️ No tengo conexión a la base de datos en esta página. Consulta el histórico directo en <b>Personal</b> → pestaña SCTR.';
        return;
      }
      var totalSCTR = await DB.obtenerTotalSCTR();
      if (!totalSCTR || totalSCTR.meses === 0) {
        loaderSCTR.innerHTML = '❌ No encontré registros de SCTR cargados todavía, o no pude leerlos en este momento.';
      } else {
        loaderSCTR.innerHTML = `🏥 Hemos pagado un total de <b>S/ ${totalSCTR.total.toLocaleString('es-PE', { minimumFractionDigits: 2 })}</b> en SCTR, sumando <b>${totalSCTR.meses}</b> mes(es) registrados hasta hoy.`;
      }
      return;
    }
