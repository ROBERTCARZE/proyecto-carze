...
(function () {
  ...
  var SYSTEM_PROMPT =
    ...
    'con humor hacia los objetivos de CARZE. 3) Para CUALQUIER pregunta sobre datos reales de ' +
    'la empresa (cifras, conteos, pendientes, totales, análisis por cualquier módulo: caja, ' +
    'SCTR, reportes, cotizaciones, certificados, facturas, presupuestos, personal, eventos, ' +
    'impuestos, seguimiento), SIEMPRE usa las herramientas disponibles para obtener el dato ' +
    'exacto — nunca inventes una cifra. Si una herramienta no tiene la información, dilo con honestidad. ' +
    '4) Conoces el propósito de los 13 módulos del sistema, úsalo para orientar al usuario aunque ' +
    'la pregunta no sea numérica: Dashboard (resumen general), Cotizaciones (presupuestos de obra ' +
    'antes de aprobarse), Certificados (valorizaciones/certificados de avance de obra para facturar), ' +
    'Presupuesto (control presupuestal general), Facturas (cobranza a clientes), Finanzas (vista ' +
    'consolidada de todo lo financiero), Caja Diaria (efectivo del día a día), Flujo de Caja ' +
    '(proyección de ingresos/egresos futuros), Pronto Pago (descuentos por pago anticipado a ' +
    'clientes), Impuestos (declaraciones y pagos a SUNAT), Personal (trabajadores y cuadrillas), ' +
    'Eventos (calendario y agenda), Vencimientos y Alertas (SCTR, EMO, facturas y préstamos por ' +
    'vencer), Seguimiento (control de avance y respuesta de proyectos/servicios).';
  ...
  var DB = {
    ...
    // Colección real: 'sctr', campos: mes ('YYYY-MM'), precioSalud, precioPension
    // Suma TODO lo pagado en SCTR desde el inicio hasta el mes actual (inclusive).
    obtenerTotalSCTR: async function () {
      if (!this.disponible()) return null;
      var F = window.CZ_FIRESTORE;
      try {
        var hoy = new Date();
        var mesActual = hoy.getFullYear() + '-' + String(hoy.getMonth() + 1).padStart(2, '0');
        var snap = await F.getDocs(F.collection(F.db, 'sctr'));
        var resultado = { total: 0, meses: 0, detalle: [] };
        snap.forEach(function (docSnap) {
          var d = docSnap.data();
          if (String(d.mes || '') > mesActual) return; // ignora meses futuros ya cargados por adelantado
          var subtotal = (parseFloat(d.precioSalud) || 0) + (parseFloat(d.precioPension) || 0);
          resultado.total += subtotal;
          resultado.meses++;
          resultado.detalle.push({ mes: d.mesLabel || d.mes, monto: subtotal });
        });
        resultado.detalle.sort(function (a, b) { return (a.mes > b.mes) ? 1 : -1; });
        return resultado;
      } catch (e) {
        console.error('[CARZECITO] Error sumando SCTR:', e);
        return null;
      }
    },

    // Genérica y GLOBAL: contar, sumar o agrupar sobre cualquiera de los
    // 8 módulos reales del ERP. Un solo punto de entrada para preguntas
    // de análisis, en vez de una función por módulo.
    MODULOS_VALIDOS: {
      ...
      personal: 'personal',
      eventos: 'eventos',
      impuestos: 'impuestos_declaraciones',
      seguimiento: 'seguimiento',
      compromisos: 'compromisos'
    },

    consultarModulo: async function (modulo, operacion, campo, valor, campoNumerico, campoAgrupar) {
      if (!this.disponible()) return null;
      var coleccionReal = this.MODULOS_VALIDOS[modulo];
      if (!coleccionReal) {
        return { error: 'Módulo no reconocido. Módulos válidos: ' + Object.keys(this.MODULOS_VALIDOS).join(', ') + '.' };
      }
      var F = window.CZ_FIRESTORE;
      try {
        var snap = await F.getDocs(F.collection(F.db, coleccionReal));
        var docs = [];
        snap.forEach(function (docSnap) {
          var d = docSnap.data();
          if (campo && valor && String(d[campo]) !== String(valor)) return;
          docs.push(d);
        });

        if (operacion === 'sumar') {
          if (!campoNumerico) return { error: 'Falta indicar campoNumerico para sumar.' };
          var total = 0;
          docs.forEach(function (d) { total += parseFloat(d[campoNumerico]) || 0; });
          return { modulo: modulo, operacion: 'sumar', campoNumerico: campoNumerico, total: total, cantidadRegistros: docs.length };
        }

        if (operacion === 'agrupar') {
          if (!campoAgrupar) return { error: 'Falta indicar campoAgrupar para agrupar.' };
          var grupos = {};
          docs.forEach(function (d) {
            var clave = (d[campoAgrupar] === undefined || d[campoAgrupar] === null || d[campoAgrupar] === '') ? 'Sin dato' : String(d[campoAgrupar]);
            grupos[clave] = (grupos[clave] || 0) + 1;
          });
          return { modulo: modulo, operacion: 'agrupar', campoAgrupar: campoAgrupar, porGrupo: grupos, totalRegistros: docs.length };
        }

        // contar (default): si no filtraron por campo/valor, desglosa por 'estado' (o 'cerrado' en seguimiento)
        if (campo && valor) {
          return { modulo: modulo, operacion: 'contar', campo: campo, valor: valor, total: docs.length };
        }
        var campoResumen = (modulo === 'seguimiento') ? 'cerrado' : 'estado';
        var resumen = {};
        docs.forEach(function (d) {
          var clave = (d[campoResumen] === undefined || d[campoResumen] === null || d[campoResumen] === '') ? 'Sin dato' : String(d[campoResumen]);
          resumen[clave] = (resumen[clave] || 0) + 1;
        });
        return { modulo: modulo, operacion: 'contar', total: docs.length, desglosePor: campoResumen, desglose: resumen };

      } catch (e) {
        console.error('[CARZECITO] Error consultando módulo ' + modulo + ':', e);
        return { error: 'No se pudo leer el módulo ' + modulo + '.' };
      }
    },

    // Colección real: 'prestamos'. Estructura especial: cada documento tiene
    // un array "cuotas" con estado por cuota (Pagado / no). Se replica la
    // misma lógica de cálculo que ya usa finanzas.html, para que el número
    // coincida exacto con lo que el usuario ve en pantalla.
    obtenerResumenPrestamos: async function () {
      if (!this.disponible()) return null;
      var F = window.CZ_FIRESTORE;
      try {
        var snap = await F.getDocs(F.collection(F.db, 'prestamos'));
        var resumen = { cantidadPrestamos: 0, activos: 0, totalPagado: 0, totalPendiente: 0, detalle: [] };
        snap.forEach(function (docSnap) {
          var p = docSnap.data();
          var cuotas = Array.isArray(p.cuotas) ? p.cuotas : [];
          var pagado = 0, pendiente = 0, cuotasPagadas = 0;
          cuotas.forEach(function (c) {
            var monto = parseFloat(c.cuota || c.apagar || 0) || 0;
            if (c.estado === 'Pagado') { pagado += monto; cuotasPagadas++; }
            else { pendiente += monto; }
          });
          resumen.cantidadPrestamos++;
          resumen.totalPagado += pagado;
          resumen.totalPendiente += pendiente;
          var estaActivo = cuotasPagadas < cuotas.length;
          if (estaActivo) resumen.activos++;
          resumen.detalle.push({
            entidad: p.entidad || 'Sin nombre',
        ...
        console.error('[CARZECITO] Error obteniendo resumen de préstamos:', e);
        return { error: 'No se pudo leer la colección de préstamos.' };
      }
    },
  ...
  /* ---------------------------------------------------------------------
     6. GEMINI — vía proxy (la key nunca toca el navegador), con
        Function Calling: Gemini decide QUÉ función necesita y con qué
        argumentos; nosotros la ejecutamos contra Firestore/DOM real y
        le devolvemos el resultado para que redacte la respuesta final.
        Para agregar una capacidad nueva a CARZECITO: 1) descríbela en
        HERRAMIENTAS_DISPONIBLES del Worker, 2) impleméntala aquí abajo
        en el objeto HERRAMIENTAS con el mismo nombre. Ya no hace falta
        escribir reglas de palabras clave para cada frase posible.
  --------------------------------------------------------------------- */
  var HERRAMIENTAS = {
    obtener_saldo_caja: async function () {
      var saldo = await DB.obtenerSaldoCaja();
      return saldo === null ? { error: 'No se pudo leer el saldo (sin conexión a Firestore en esta página).' } : { saldoSoles: saldo };
    },
    obtener_total_sctr: async function () {
      var r = await DB.obtenerTotalSCTR();
      return r || { error: 'No se pudo leer el histórico de SCTR.' };
    },
    obtener_reporte_mensual: async function (args) {
      var mes = (args && args.mes) || mesActualStr();
      var r = await DB.obtenerReporteMensual(mes);
      return r || { error: 'No se pudo generar el reporte de ese mes.' };
    },
    obtener_alertas_vencimientos: async function () {
      return obtenerResumenAlertas() || { total: 0, detalle: [] };
    },
    registrar_gasto: async function (args) {
      var monto = args && args.monto;
      var concepto = (args && args.concepto) || 'Gasto registrado vía CARZECITO';
      if (!monto) return { error: 'Falta el monto.' };
      var ok = await DB.registrarGastoRapido(monto, concepto);
      return ok ? { exito: true, monto: monto, concepto: concepto } : { error: 'No se pudo registrar el gasto.' };
    },
    consultar_modulo: async function (args) {
      var modulo = args && args.modulo;
      if (!modulo) return { error: 'Falta indicar el módulo.' };
      var r = await DB.consultarModulo(
        modulo.toLowerCase().trim(),
        (args && args.operacion) || 'contar',
        args && args.campo,
        args && args.valor,
        args && args.campoNumerico,
        args && args.campoAgrupar
      );
      return r || { error: 'No se pudo consultar ese módulo.' };
    },
    obtener_resumen_prestamos: async function () {
      var r = await DB.obtenerResumenPrestamos();
      return r || { error: 'No se pudo leer los préstamos.' };
    }
  };
  ...
  async function consultarGemini(promptUsuario) {
    ...
    try {
      ...
      var primeraVuelta = await llamarProxy(basePayload);
      if (primeraVuelta.error) return primeraVuelta;

      // Gemini no pidió ninguna función: respuesta directa
      if (!primeraVuelta.funcion) {
        return { texto: primeraVuelta.respuesta || null };
      }

      // Gemini pidió ejecutar una herramienta
      var nombreFn = primeraVuelta.funcion;
      var argsFn = primeraVuelta.argumentos || {};
      var firmaFn = primeraVuelta.thoughtSignature || null;
      var ejecutor = HERRAMIENTAS[nombreFn];

      if (!ejecutor) {
        console.error('[CARZECITO] Gemini pidió una función desconocida:', nombreFn);
        return { error: 'proxy' };
      }

      var resultadoFn = await ejecutor(argsFn);

      // Segunda vuelta: le pasamos el resultado real (+ la firma exacta que
      // Gemini nos dio) para que redacte la respuesta final.
      var segundaVuelta = await llamarProxy(Object.assign({}, basePayload, {
        llamadaFuncion: { nombre: nombreFn, argumentos: argsFn, thoughtSignature: firmaFn },
        resultadoFuncion: resultadoFn
      }));

      if (segundaVuelta.error) return segundaVuelta;
      return { texto: segundaVuelta.respuesta || null };
  ...
  var VERBOS_NAVEGACION = ['ir a', 'ir al', 'vamos a', 'vamos al', 'llevame a', 'llevame al',
    'muestrame', 'muestra el', 'muestra la', 'abre', 'abrir', 've a', 've al',
    'quiero ir', 'navega a', 'navegar a', 'entrar a', 'entra a'];

  function interpretarComandoEstatico(textoOriginal) {
    var texto = normalizar(textoOriginal);
    if (!texto || !MAPA_PAGINAS) return null;

    // Solo se considera navegación si: (a) el mensaje es EXACTAMENTE el
    // nombre de un módulo (ej. clic en un chip: "facturas"), o (b) trae un
    // verbo explícito de navegación ("ir a", "vamos a", etc.). Así una
    // pregunta como "¿cómo vamos con el flujo de caja?" NO te saca de la
    // página — solo una orden clara de moverse lo hace.
    var esOrdenExplicita = VERBOS_NAVEGACION.some(function (v) { return texto.indexOf(v) !== -1; });
    ...
    MAPA_PAGINAS.forEach(function (entrada) {
      entrada.claves.forEach(function (clave) {
        var esCoincidenciaExacta = (texto === clave);
        if (!esCoincidenciaExacta && !esOrdenExplicita) return; // pregunta suelta: no navega
  ...
  async function procesarMensaje(textoOriginal) {
    ...
    // B. Navegación entre módulos (atajo rápido, sin pasar por IA)
    ...
    // C. Todo lo demás: Gemini decide si responde directo o necesita datos
    // reales (saldo, SCTR, reportes, vencimientos) vía Function Calling.
    ...
    if (resultado.texto) {
      ...
      loaderIA.innerHTML = 'Todavía no tengo mi conexión de IA configurada (falta la URL del proxy en <code>CONFIG.proxyUrl</code>). Mientras tanto dime <b>"ir a personal"</b> u otro comando de navegación.';
    } else {
      loaderIA.innerHTML = 'No pude conectarme a mi cerebro de IA en este momento 🤔 Intenta de nuevo en unos segundos, o dime un comando como <b>"ir a personal"</b>.';
    }
  }
