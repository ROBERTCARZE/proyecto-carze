...
import { collection, query, orderBy,
         ...
         getDocs, doc, serverTimestamp, writeBatch } from "https://www.gstatic.com/firebasejs/11.8.1/firebase-firestore.js";
import { ref, uploadBytes, getDownloadURL, deleteObject } from "https://www.gstatic.com/firebasejs/11.8.1/firebase-storage.js";
...
var pendingImportCuentas=null; // filas ya parseadas y validadas, listas para subir

// Encabezados esperados (deben coincidir con el mismo orden que exportarCuentasExcel)
var CUENTAS_HEADERS=['Apellidos y Nombres','Entidad Financiera','Número de Cuenta','CCI','Billetera Digital','Número'];

function manejarImportCuentas(event){
    var file=event.target.files[0];
    if(!file) return;
    ...
    reader.onload=function(e){
        try{
            ...
            var rows=XLSX.utils.sheet_to_json(ws,{header:1,raw:false,blankrows:false,defval:''});

            if(!rows.length){
                toast('El archivo Excel está vacío','err');
                event.target.value='';
                return;
            }

            // Detectar si la primera fila es encabezado (contiene "Apellidos" o similar)
            var startIdx=0;
            var primera=(rows[0]||[]).map(function(c){return String(c||'').trim().toLowerCase();});
            if(primera.some(function(c){return c.indexOf('apellido')>-1||c.indexOf('nombre')>-1;})){
                startIdx=1;
            }
            ...
            for(var i=startIdx;i<rows.length;i++){
                var r=rows[i]||[];
                var nombreCompleto=String(r[0]||'').trim();
                if(!nombreCompleto){omitidas++;continue;}
                filas.push({
                    nombreCompleto:nombreCompleto,
                    entidadFinanciera:String(r[1]||'').trim()||'-',
                    numeroCuenta:String(r[2]||'').trim()||'-',
                    cci:String(r[3]||'').trim()||'-',
                    billeteraDigital:String(r[4]||'').trim()||'-',
                    numero:String(r[5]||'').trim()||'-'
                });
            }

            if(!filas.length){
                toast('No se encontraron filas válidas para importar','err');
                event.target.value='';
                return;
            }

            // Evitar duplicados exactos respecto a lo ya registrado en Firestore
            var nuevas=[], duplicadas=0;
            filas.forEach(function(f){
                var yaExiste=cuentasData.some(function(c){
                    return String(c.nombreCompleto||'').trim().toLowerCase()===f.nombreCompleto.toLowerCase()
                        && String(c.numeroCuenta||'').trim()===f.numeroCuenta
                        && String(c.entidadFinanciera||'').trim()===f.entidadFinanciera;
                });
                if(yaExiste) duplicadas++; else nuevas.push(f);
            });

            pendingImportCuentas=nuevas;

            var msg='Se encontraron <b>'+filas.length+'</b> fila'+(filas.length!==1?'s':'')+' en el archivo.<br>';
            msg+='<b>'+nuevas.length+'</b> se registrarán como nuevas cuentas.<br>';
            if(duplicadas) msg+='<b>'+duplicadas+'</b> ya existen y se omitirán (mismo nombre, entidad y número de cuenta).<br>';
            if(omitidas) msg+='<b>'+omitidas+'</b> fila'+(omitidas!==1?'s':'')+' sin nombre se omitieron.';

            if(!nuevas.length){
                toast('No hay cuentas nuevas para importar (todas ya existen)','warn');
                event.target.value='';
                return;
            }

            document.getElementById('confirmMsg').innerHTML=msg;
            document.getElementById('confirmOkBtn').textContent='Sí, importar';
            document.getElementById('confirmOverlay').classList.add('open');
            document.getElementById('confirmOkBtn').onclick=ejecutarImportCuentas;
        }catch(err){
...
window.manejarImportCuentas=manejarImportCuentas;

async function ejecutarImportCuentas(){
    if(!pendingImportCuentas || !pendingImportCuentas.length){cerrarConfirm();return;}
    var filas=pendingImportCuentas;
    pendingImportCuentas=null;
    cerrarConfirm();
    document.getElementById('confirmOkBtn').textContent='Sí, eliminar'; // restaurar texto original

    toast('Importando '+filas.length+' cuenta'+(filas.length!==1?'s':'')+'...','ok');
    try{
        // Firestore permite máx. 500 operaciones por batch
        var CHUNK=450;
        for(var i=0;i<filas.length;i+=CHUNK){
            var batch=writeBatch(db);
            filas.slice(i,i+CHUNK).forEach(function(f){
                var ref=doc(collection(db,COL_CUENTAS));
                batch.set(ref,{
                    nombreCompleto:f.nombreCompleto,
                    entidadFinanciera:f.entidadFinanciera,
                    numeroCuenta:f.numeroCuenta,
                    cci:f.cci,
                    billeteraDigital:f.billeteraDigital,
                    numero:f.numero,
        ...
        toast(filas.length+' cuenta'+(filas.length!==1?'s':'')+' importada'+(filas.length!==1?'s':'')+' ✓','ok');
    }catch(err){
