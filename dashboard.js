...
// Producción mensual: cuántas cotizaciones y certificados se aprobaron
// (estado 'Facturado') por mes de la fecha del documento, y cuánto sumó
// esa producción en S/ (subtotal de ambos módulos combinados).
function aprobadosDelMes(lista, mk){
    return lista.filter(function(r){
        return mesKey(r.fecha)===mk && (r.estado||'').toLowerCase()==='facturado';
    });
}
function renderChartProduccion(){
    var meses=ultimosMeses(8);
    var cotCant=[], certCant=[], produccion=[];
    meses.forEach(function(mk){
        var cot=aprobadosDelMes(G.cotizaciones,mk);
        var cert=aprobadosDelMes(G.certificados,mk);
        cotCant.push(cot.length);
        certCant.push(cert.length);
        var totCot =cot.reduce(function(s,r){return s+(parseFloat(r.subtotal)||0);},0);
        var totCert=cert.reduce(function(s,r){return s+(parseFloat(r.subtotal)||0);},0);
        produccion.push(totCot+totCert);
    });
    var labels=meses.map(function(k){var p=k.split('-');return MESES[parseInt(p[1])-1]+' '+p[0].substring(2);});

    var maxIdx=0;
    produccion.forEach(function(v,i){ if(v>produccion[maxIdx]) maxIdx=i; });
    var subEl=document.getElementById('chartProdSub');
    if(subEl){
        subEl.innerHTML='Cantidad aprobada (Facturado) por mes · línea = producción en S/'+
            (produccion[maxIdx]>0?' · <b style="color:var(--naranja)">Mes de mayor producción: '+labels[maxIdx]+' (S/ '+fmt(produccion[maxIdx],0)+')</b>':'');
    }
    ...
    chartProduccion=new Chart(canvas,{
        type:'bar',
        data:{labels:labels,datasets:[
            {type:'bar',label:'Cotizaciones aprobadas',data:cotCant,backgroundColor:'rgba(30,64,175,.75)',borderRadius:4,borderSkipped:false,order:2,yAxisID:'yCant'},
            {type:'bar',label:'Certificados aprobados',data:certCant,backgroundColor:'rgba(22,163,74,.75)',borderRadius:4,borderSkipped:false,order:2,yAxisID:'yCant'},
            {type:'line',label:'Producción (S/)',data:produccion,
                borderColor:'#f97316',backgroundColor:'#f97316',borderWidth:2.5,
                pointBackgroundColor:'#f97316',pointRadius:4,pointHoverRadius:6,tension:0.25,fill:false,order:1,yAxisID:'ySoles'}
        ]},
        options:{responsive:true,maintainAspectRatio:false,
            interaction:{mode:'index',intersect:false},
            plugins:{legend:{position:'bottom',labels:{boxWidth:10,font:{size:9,family:'Verdana'}}},
                tooltip:{callbacks:{label:function(c){
                    if(c.dataset.yAxisID==='ySoles') return ' '+c.dataset.label+': S/ '+fmt(c.parsed.y||0,0);
                    return ' '+c.dataset.label+': '+(c.parsed.y||0);
                }}}},
            scales:{
                x:{grid:{display:false},ticks:{font:{size:9,family:'Verdana'}}},
                yCant:{position:'left',beginAtZero:true,grid:{color:'rgba(0,0,0,.04)'},ticks:{font:{size:9,family:'Verdana'},precision:0},title:{display:true,text:'Cantidad',font:{size:9,family:'Verdana'}}},
                ySoles:{position:'right',beginAtZero:true,grid:{display:false},ticks:{font:{size:9,family:'Verdana'},callback:function(v){return 'S/'+Math.round(v/1000)+'k';}},title:{display:true,text:'Producción (S/)',font:{size:9,family:'Verdana'}}}
            }}
    });
}
